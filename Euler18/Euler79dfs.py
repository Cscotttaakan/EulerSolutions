...
class Graph:
    ...
    def createTable(self):#link nodes
        ...
        for i in range(len(self.table)-1):
            self.tableDict[self.table[i][0]].append(self.table[i][1])
            self.tableDict[self.table[i][0]].append(self.table[i][2])
            self.tableDict[self.table[i][1]].append(self.table[i][2])
                #for x in range(2):
